...
(define (domain fn-counters)
    ...
    (:functions
        (value_counter ?c - counter);; - int  ;; The value shown in counter ?c
    ...
    (:action increment
         ...
         :precondition (and (<= (+ (value_counter ?c) 1) (max_int)))
         :effect (and (increase (value_counter ?c) 1))
    )

    ;; Decrement the value in the given counter by one
    (:action decrement
         :parameters (?c - counter)
         :precondition (and (>= (value_counter ?c) 1))
         :effect (and (decrease (value_counter ?c) 1))
    )
)
