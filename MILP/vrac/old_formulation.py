...
def build_model(solver_name, T):
    ...
    for p in Vp:
        for t in Tau:
            m += mip.xsum(u[a][t] for a in pnd[p]) >= u_pa[p][t]
            for a in pnd[p]:
                m += u[a][t] <= u_pa[p][t]
                
            m += mip.xsum(u[a][t] for a in anp[p]) >= u_a[p][t]
            for a in anp[p]:
                m += u[a][t] <= u_a[p][t]
                
            m += mip.xsum(u[a][t] for a in pd[p]) == u_pd[p][t]
    ...
    for p in Vp:
        for t in Tau:
            m += u_pa[p][t] + u_m[p][t] + u_pd[p][t] <= u_a[p][t-1] + u_pa[p][t-1] + u_m[p][t-1]
