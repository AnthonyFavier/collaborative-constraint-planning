...
def Decompose(state: DecompositionState):
    ...
    SYSTEM_PROMPT = (
        ...
        "{pddl_problem}\n"
        "</pddl_problem>\n"
        "Your goal is to generate a clear, non ambiguous, list of natural language constraints properly capturing the user intent. "
        "These constraints can't directly affect actions (e.g. robot1 must not go in location2). Instead, the constraint must focus on facts (e.g. robot1 must never be in location2). "
        "For this purpose, you can rephrase the user input, and decompose it into simpler sub-constraints. "
