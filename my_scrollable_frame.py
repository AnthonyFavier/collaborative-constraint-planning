...
class MyScrollableFrame(tkinter.Frame, CTkAppearanceModeBaseClass, CTkScalingBaseClass):
    def __init__(self,
                 master: Any,
                 width: int = 200,
                 height: int = 200,
        ...
        if self._orientation == "horizontal":
            self._scrollbarh = CTkScrollbar(master=self._parent_frame, orientation="horizontal", command=self._parent_canvas.xview,
                                           fg_color=scrollbar_fg_color, button_color=scrollbar_button_color, button_hover_color=scrollbar_button_hover_color)
            self._parent_canvas.configure(xscrollcommand=self._scrollbarh.set)
        elif self._orientation == "vertical":
            self._scrollbarv = CTkScrollbar(master=self._parent_frame, orientation="vertical", command=self._parent_canvas.yview,
                                           fg_color=scrollbar_fg_color, button_color=scrollbar_button_color, button_hover_color=scrollbar_button_hover_color)
            self._parent_canvas.configure(yscrollcommand=self._scrollbarv.set)
    ...
    def _create_grid(self):
        ...
        if self._orientation == "horizontal":
            ...
            self._scrollbarh.grid(row=2, column=0, sticky="nsew", padx=border_spacing)
            ...
            self._scrollbarv.grid(row=1, column=1, sticky="nsew", pady=border_spacing)
    ...
    def configure(self, **kwargs):
        ...
        if "scrollbar_fg_color" in kwargs:
            if self._orientation in ["vertical", "both"]:
                self._scrollbarv.configure(fg_color=kwargs.pop("scrollbar_fg_color"))
            if self._orientation in ["horizontal", "both"]:
                self._scrollbarh.configure(fg_color=kwargs.pop("scrollbar_fg_color"))

        if "scrollbar_button_color" in kwargs:
            if self._orientation in ["vertical", "both"]:
                self._scrollbarv.configure(button_color=kwargs.pop("scrollbar_button_color"))
            if self._orientation in ["horizontal", "both"]:
                self._scrollbarh.configure(button_color=kwargs.pop("scrollbar_button_color"))

        if "scrollbar_button_hover_color" in kwargs:
            if self._orientation in ["vertical", "both"]:
                self._scrollbarv.configure(button_hover_color=kwargs.pop("scrollbar_button_hover_color"))
            if self._orientation in ["horizontal", "both"]:
                self._scrollbarh.configure(button_hover_color=kwargs.pop("scrollbar_button_hover_color"))
    ...
    def cget(self, attribute_name: str):
        if attribute_name == "width":
            ...
            scrollbar = self._scrollbarv if self._orientation=="vertical" else self._scrollbarh
            return scrollbar.cget("fg_color")
        elif attribute_name.startswith("scrollbar_button_color"):
            scrollbar = self._scrollbarv if self._orientation=="vertical" else self._scrollbarh
            return scrollbar.cget("button_color")
        elif attribute_name.startswith("scrollbar_button_hover_color"):
            scrollbar = self._scrollbarv if self._orientation=="vertical" else self._scrollbarh
            return scrollbar.cget("button_hover_color")
